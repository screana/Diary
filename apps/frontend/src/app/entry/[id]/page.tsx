...
export default async function EntryPage({ params }: PageProps) {
  const entry: Entry | undefined = await entriesApi.get(params.id);

  if (!entry) {
    return (
      <div className="bg-white rounded shadow-sm p-4">
        <div className="text-sm text-gray-500">この日記は見つかりませんでした。</div>
  ...
  return (
    <article className="bg-white rounded shadow-sm p-5 space-y-4">
      ...
      {entry.worlds && entry.worlds.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold mb-2 text-gray-700">行ったワールド</h2>
